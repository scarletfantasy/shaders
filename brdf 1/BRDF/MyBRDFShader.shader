    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Schlick_F(half3 R, half cosA)
            {
                ...
                float3 result=R+(1-R)*pow((1-cosA),5);
            ...
            float GGX_D(float roughness, float NdotH)
            {
                ...
                float result=(roughness*roughness)/(UNITY_PI*pow((pow(NdotH,2)*(roughness*roughness-1)+1),2));
                return result;
            }

            float CookTorrence_G (float NdotL, float NdotV, float VdotH, float NdotH){
                //// TODO: your implementation
                float result=min(1,min(2*NdotH*NdotV/VdotH,2*NdotH*NdotL/VdotH));
                return result;
            }
